...
//check the result with groundtruth
void
...
//#pragma omp parallel for
    for (int i = 0; i < qsize; i++) {
        ...
        if(i == 10 || i==11)
          cout<<endl;
        //jie: check result size and groundtruth size
        //cout<<"KANN result size is "<<result.size()<<endl;
        //cout<<"groundtruth size is "<<gt.size()<<endl;
        ...
        //cout<<"query "<<i<<" : "<< time_us_per_query<< endl;
...
void test_vs_recall(float *massQ, size_t vecsize, size_t qsize, HierarchicalNSW<float> &appr_alg, size_t vecdim,
    ...
    //jie: test performance distributuion
    size_t ef = 10;
    appr_alg.setEf(ef);
    float recall = test_approx(massQ, vecsize, qsize, appr_alg, vecdim, answers, k);
...
void sift_test() {
    ...
    ifstream inputQA("sift/sift_groundtruth.ivecs", ios::binary);
    //ifstream inputQA("../../1M_d=4qa.bin", ios::binary);
    inputQA.read((char *) massQA, qsize * 100 * sizeof(int));
    inputQA.close();

    int maxn = 16;  //jie: what's this for???
...
//#define LOAD_I
    ...
    //jie:
    HierarchicalNSW<float> appr_alg(&l2space, vecsize, 48, 400);
