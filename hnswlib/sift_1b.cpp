...
get_gt(unsigned int *massQA, unsigned char *massQ, unsigned char *mass, size_t vecsize, size_t qsize, L2SpaceI &l2space,
    ...
    cout << qsize << "\n";
    //jie: check groundtruth
    //ofstream myfile;
    //myfile.open ("readgt.txt");
    for (int i = 0; i < qsize; i++) {
        for (int j = 0; j < k; j++) {
            //answers[i].emplace(i, massQA[1000 * i + j]);
            //float other = fstdistfunc_(massQ + i * vecdim, mass + massQA[1000 * i + j],
            //                           l2space.get_dist_func_param());
            answers[i].emplace(0.0f, massQA[1000 * i + j]);
            //answers[i].emplace(other, massQA[1000 * i + j]);
        }

        /*for (int j = 0; j < 1000; j++) {
            myfile<<answers[i].first<<" "<<answers[i].second<<" ";
        }
        myfile<<endl;*/
    }
    //myfile.close();
...
*/
    for (int i = 0; i < qsize; i++) {
        ...
        while (gt.size()) {
            ...
            //myfile<<gt.top().first<<" "<<gt.top().second<<" ";
            gt.pop();
...
void sift_test1B() {
  ...
  int M = 16;
    ...
    char path_gt[1024]; //jie: GT stands for ground truth?
    //char *path_gt = "groundtruth.bin";
...
/*
    ...
    inputGT.read((char *) massQA, qsize * 100 * sizeof(int));
    inputGT.close();
*/
    ...
    if (exists_test(path_index)) {
        ...
        appr_alg->addPoint((void *) (massb), (size_t) 0); //data point, label 0 for enter point
        int j1 = 0;
        StopW stopw = StopW();
...
#pragma omp parallel for
        for (int i = 1; i < vecsize; i++) {
            ...
            int j2=0;
#pragma omp critical
            {//here synchronize again! the label is element id?!
