...
int main()
{
	std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> test;
	test.emplace(0.2, 1);
	test.emplace(0.5, 2);
	test.emplace(0.1, 10);
	test.emplace(1.1, 37);
	while(!test.empty())
	{
		std::cout<<test.top().first<<" "<<test.top().second<<std::endl;
		test.pop();
	}
	return 0;
}
...
//        static const unsigned char REUSE_MARK = 0x10;
      ...
      */
        ...
        //void promote node from level 0 to level 1
        //provide internal id for promotion
        bool exam_promotePoint(tableint cur_id, int level)
        {
        ...
        void Count_in_degree()
        {
            ...
            for(size_t i=0; i<max_elements_; i++)
            {
               ...
               {
                  largerthan++;
                  if(exam_promotePoint(i,1))
                      canpromote++;
               }
            }
            cout<<"large than 600 nodes "<<largerthan<<" protentially promote node is "<<canpromote<<endl;
        }
